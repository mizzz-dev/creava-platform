...
## 新構成
- API: `POST /api/forms/upload` / `POST /api/forms/confirm` / `POST /api/forms/submit`
...
- 最大件数: 3件（`INQUIRY_MAX_FILES`）
- 合計サイズ: 20MB（`INQUIRY_MAX_TOTAL_FILE_BYTES`）
- 危険拡張子は MIME ベースで拒否
