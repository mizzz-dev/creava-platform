...
| POST | `/api/payments/fanclub/checkout-session` | FC決済開始 | 必須(Logto access token) |
...
- 入力: `planId`, `locale`
- 検証: Logto token, planのjoinable, stripePriceId
- 出力: `url`, `sessionId`

### 2.3 Webhook
- 入力: Stripe event（署名必須）
- 処理: 重複防止ログ→payment/subscription/order/revenue記録→processed更新

### 2.4 Internal Revenue Summary
- クエリ: `sourceSite?`, `reportPeriod?`, `limit?`
- 出力:
  - totals: `gross/net/refund/shipping/discount/tax`
  - counts: `failed/canceled/refunded`
  - `bySourceSite`, `byRevenueType`, `daily`, `monthly`
